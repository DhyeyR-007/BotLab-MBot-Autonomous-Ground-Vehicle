...
private:
    // float pid[3] = {1.8, 3.0, 1.5* 1E-4}; //High Speed
    // float pid[3] = {3, 0, 0}; //High Speed

    float pid[3] = {1.0, 2.5, 0.0}; //Low Speed
...
public:
    ...
    virtual mbot_lcm_msgs::twist2D_t get_command(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target) override
    {
        ...
        if (fwd_vel - last_speed > 0.008){
            fwd_vel = last_speed + 0.008;
        }
        last_speed = fwd_vel;
        printf("fwd_vel: %f\n", fwd_vel);
...
int main(int argc, char** argv)
{
    ...
    while(true)
    {
    	...
    	if(controller.timesync_initialized()){
            ...
            // Fwd vel
            float max_speed = 0.2;
            if (cmd.vx > max_speed) cmd.vx = max_speed;
            else if (cmd.vx < -max_speed) cmd.vx = -max_speed;
