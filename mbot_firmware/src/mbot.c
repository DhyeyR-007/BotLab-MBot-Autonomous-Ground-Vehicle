...
#define PID_TAU MAINLOOP_DELAY_MS / 250.0
...
bool mbot_loop(repeating_timer_t *rt)
{
    ...
    fprintf(odometry_log_file, "%llu,%.3f,%.3f,%.3f\n", mbot_odometry.utime, mbot_odometry.x, mbot_odometry.y, mbot_odometry.theta);
    fflush(odometry_log_file); // 确保数据立即被写入文件
    ...
    if (global_comms_status == COMMS_OK)
    {
        ...
        if(drive_mode == MODE_MOTOR_VEL_OL){
            ...
            PIDController_Update(&pid_L, mbot_motor_vel_cmd.velocity[params.mot_left], mbot_motor_vel.velocity[params.mot_left]);
            PIDController_Update(&pid_R, mbot_motor_vel_cmd.velocity[params.mot_right], mbot_motor_vel.velocity[params.mot_right]);

            // printf("Goal (rad/s): \t[%.3f, %.3f]\n", mbot_motor_vel_cmd.velocity[params.mot_left], mbot_motor_vel_cmd.velocity[params.mot_right]);
            // printf("Real (rad/s): \t[%.3f, %.3f]\n", mbot_motor_vel.velocity[params.mot_left], mbot_motor_vel.velocity[params.mot_right]);
            // printf("PID (rad/s): \t[%.3f, %.3f]\n", pid_L.out, pid_R.out);
            // printf("v (m/s): \t[%.3f, %.3f]\n", mbot_vel_cmd.vx, -mbot_vel_cmd.vx);


            mbot_motor_pwm.utime = global_utime;
            mbot_motor_pwm_cmd.pwm[params.mot_left] = _calibrated_pwm_from_vel_cmd(pid_L.out * params.motor_polarity[params.mot_left], params.mot_left);
            mbot_motor_pwm_cmd.pwm[params.mot_right] = _calibrated_pwm_from_vel_cmd(pid_R.out* params.motor_polarity[params.mot_right], params.mot_right);
            }
        }else {
            drive_mode = MODE_MOTOR_PWM;
            mbot_motor_pwm.utime = global_utime;
...
int main()
{   
    FILE *odometry_log_file = fopen("/path/to/odometry_log.csv", "w"); // 修改为您的路径
    if (odometry_log_file == NULL) {
        // 文件打开失败的错误处理
        perror("Error opening odometry log file");
        return 1;
    }
    fprintf(odometry_log_file, "Timestamp,X,Y,Theta\n"); // 写入文件头部
